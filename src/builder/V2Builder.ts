...
export function parseEmoji(input: string): { name?: string; id?: string; animated?: boolean } {
...
export class V2Builder {
  ...
  static parse(input: unknown): V2Builder {
    ...
    if (roots.length === 1 && isContainer(roots[0])) {
      ...
      builder.containerData.components = roots as APIContainerComponent["components"];
    }
    return builder;
  }
  ...
  text(content: string): this {
    this.containerData.components.push({ type: ComponentType.TextDisplay, content });
    return this;
  }
  ...
  separator(size: SeparatorSpacingSize = SeparatorSpacingSize.Small, divider = true): this {
  ...
  section(params: SectionOptions): this {
    ...
    if (params.button) {
      ...
      section.accessory = {
        ...
        ...(params.thumbnailDescription ? { description: params.thumbnailDescription } : {}),
  ...
  file(bufferOrPayload: Buffer | string | FilePayload, maybeName?: string, spoiler = false): this {
    let payload: FilePayload;

    if (typeof bufferOrPayload === "object" && !Buffer.isBuffer(bufferOrPayload)) {
  ...
  selectMenu = {
    ...
    user: (params: SelectParams): this =>
      this.pushSelect({ type: ComponentType.UserSelect, custom_id: params.customId }, params),

    role: (params: SelectParams): this =>
      this.pushSelect({ type: ComponentType.RoleSelect, custom_id: params.customId }, params),

    mentionable: (params: SelectParams): this =>
      this.pushSelect({ type: ComponentType.MentionableSelect, custom_id: params.customId }, params),
      ...
      this.pushSelect(
        {
          ...
          custom_id: params.customId,
          ...(params.channelTypes ? { channel_types: params.channelTypes } : {}),
        },
        params,
      ),
  };
  ...
  private pushSelect(
    ...
    const menu = {
      ...
      ...(params.minValues !== undefined ? { min_values: params.minValues } : {}),
      ...(params.maxValues !== undefined ? { max_values: params.maxValues } : {}),
  ...
  disableButtons(customIds?: string | string[]): this {
    opDisableButtons(this.containerData.components, { customIds, disabled: true });
    return this;
  }
  ...
  find(selector: ComponentSelector): ComponentRef | null {
    return findComponent(this.containerData.components as RawComponent[], selector);
  }

  findAll(selector: ComponentSelector): ComponentRef[] {
    return findComponents(this.containerData.components as RawComponent[], selector);
  }
  ...
  async send(target: SendTarget, options: SendOptions = {}): Promise<unknown> {
    ...
    if (typeof target.reply === "function" && !target.replied && !target.deferred) {
    ...
    throw new Error("send(): unsupported target (no reply/editReply/edit method found)");
  }
}
...
function buildGalleryItem(item: string | MediaItemOptions): APIMediaGalleryItem {
